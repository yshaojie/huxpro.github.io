...
##### 镜像准备
```bash
kubeadm config images list | grep "k8s.gcr.io" > /tmp/pull_image.sh
sed -i s/"k8s.gcr.io"/"docker\ pull\ registry.aliyuncs.com\/google_containers"/g /tmp/pull_image.sh 
sh /tmp/pull_image.sh
rm -f /tmp/pull_image.sh
```
